...
export function DataStatus() {
  ...
  return <section>
    ...
    <div style={{ overflowX: "auto" }}><table><thead><tr><th>Instrument</th><th>Interval</th><th>First candle</th><th>Last candle</th><th>Count</th><th>Raw interval gaps</th></tr></thead><tbody>{coverage.coverage.map((item) => <tr key={`${item.instrument_id}-${item.timeframe}`}><td>{item.trading_symbol}{item.is_synthetic ? " (fixture)" : ""}</td><td>{item.timeframe}</td><td>{new Date(item.first_candle).toLocaleString()}</td><td>{new Date(item.last_candle).toLocaleString()}</td><td>{item.candle_count}</td><td>{item.raw_gap_count}</td></tr>)}</tbody></table></div>
